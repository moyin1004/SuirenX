...
本轮优先资产信息扩展、体验适配、数据保障；新增可选本地模式和日用品保质期工具，移除心愿与趋势页。执行顺序与验收见 [下一阶段计划](next-phase-plan.md)。以下均为待实现任务，历史完成记录保留。

### M1：本地模式与数据保障基础

- [x] 首次启动提供「本地使用 / 连接服务器」，本地使用无需地址、账号或网络；保留已有远程配置用户的启动行为。2026-09-06：模拟器验证本地切换后资产库与远程数据隔离。
- [x] 在引入 Room 前明确本地数据为本地模式唯一事实来源、模式隔离、切换期间请求隔离与迁移规则。见 `docs/local-mode.md` 与 `AGENTS.md`。
...
- [x] 明确首版身份边界与远程备份方式；账号、跨设备同步和远程离线缓存另行设计（见 `docs/m5-sync-design.md`）。
...
- [x] 设计身份认证、数据归属、授权隔离与退出登录后设备数据保留规则。见 `docs/m5-sync-design.md`。
