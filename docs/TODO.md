...
- [x] 将 SQL 合并到唯一 001，并更新 AGENTS.md 开发阶段规则；Go 测试通过。
- [x] 合并本地/同步设计与旧阶段计划、检查点，删除四份重复文档。
- [x] 资产与用品删除入口、确认提示、本地删除及旧同步库 tombstone 入队。
- [x] 解除旧同步仓储日常读写的网络等待，显式同步超时与防重复，响应事务保护本地待发送修改。
- [x] 统一本地事实来源与旧库迁移，服务端移除业务 CRUD 契约/路由。
- [x] 接入每次修改后同步和系统后台定时调度。
- [x] 用户确认保守冲突方案，已实现单条冲突隔离与保留双方。
- [ ] 双设备真实 HTTP、后台调度和旧库升级验收。
- [x] OpenDesign 设置页原型：2026-09-08 使用 Local Codex 在现有 `suirenx-asset-redesign.html` 中完成同步、账号、冲突、备份恢复、外观和提醒流程；脚本与交互静态检查通过。
- [x] 2026-09-08 按用户确认落地原生 Android 设置页：同步状态与频率、账号与服务器、首次同步预览、逐条冲突差异、备份恢复、主题和临期设置；绑定真实数据，保存连接/登录不会自动上传。
- [x] 移除总览、工具、设置主标题上方眉标；设置二级页支持系统返回。
- [ ] 真机双端联调账号、首次同步和冲突处理；现有设备测试使用受控 transport。

实施方案与当前边界见 [数据与同步](data-sync.md)。2026-09-08 验证：Debug 构建、27 项资产单元测试、8 项同步/Room 设备测试、Go 全包测试、Proto、重复生成、diff check 通过。模拟器应用与 WorkManager 启动正常；真实双设备验收仍待完成。
...
## P1：下一阶段交付（2026-09-06 调整）

本轮优先资产信息扩展、体验适配、数据保障；新增可选本地模式和日用品保质期工具，移除心愿与趋势页。执行顺序与验收见 [下一阶段计划](data-sync.md)。以下为历史阶段的完成记录；当前改造见文首清单。

### M1：本地模式与数据保障基础

- [x] 首次启动提供「本地使用 / 连接服务器」，本地使用无需地址、账号或网络；保留已有远程配置用户的启动行为。2026-09-06：模拟器验证本地切换后资产库与远程数据隔离。
- [x] 在引入 Room 前明确本地数据为本地模式唯一事实来源、模式隔离、切换期间请求隔离与迁移规则。见 `docs/data-sync.md` 与 `AGENTS.md`。
- [x] 增加本地资产仓储，覆盖新增、详情、编辑、退役/恢复、归档/恢复、图标与总览；复用领域规则并验证与服务端计算一致。
- [x] 设置页支持模式切换；切换不删除、上传或合并数据，远程连接失败不自动转为本地模式。
- [x] 实现带格式版本的本地完整备份与恢复，涵盖资产、用品和归档记录；恢复前校验、预览、自动备份，失败整批回滚。
- [x] 明确首版身份边界与远程备份方式；账号、跨设备同步和远程离线缓存另行设计（见 `docs/data-sync.md`）。

### M2：资产信息扩展

- [x] 保留现有整数分价格，增加可选购买渠道、保修截止日、备注、标签；明确长度、日期、标签去重及清空语义。
- [x] 同步扩展领域模型、Proto/HTTP JSON、Go 校验与追加 SQL 迁移、Android DTO 映射和本地 Room 迁移。
- [x] 新建/编辑表单与详情展示新字段；兼容旧记录与旧客户端，归档资产继续只读。
- [x] 扩展备份格式兼容及业务、契约、迁移测试。

### M3：工具页与保质期管理

- [x] 底部导航调整为「资产 / 工具 / 设置」，删除心愿、趋势入口与占位页，保持导航状态与新增入口行为正确。
- [x] 新增工具入口页与日用品保质期功能：录入、编辑、详情、列表、用完/丢弃、归档与恢复。
- [x] 支持名称、分类、到期日、可选开封日与开封后有效天数、存放位置和备注；按规则计算实际到期日。
- [x] 实现已过期、今日到期、临期、正常分组及筛选，优先显示已过期用品。
- [x] 实现过期强提示：应用内持续摘要、工具 Tab 数量标记、列表与详情明确文字/图标；提供逐项处理入口。
- [x] 增加用户可配置临期天数（1–30 天，默认 7 天）；系统通知作为可选增强暂不启用，应用内提示不受影响。
- [x] 首版用品存于设备本地，任意资产模式均可使用，页面明确「保存在本机」；纳入备份恢复。远程用品存储/同步留至 M5 设计。
- [x] 增加日期边界、跨日刷新、用品处理与恢复、数据持久化相关领域验证。

### M4：体验适配与无障碍（贯穿 M1–M3，最后统一验收）

- [x] 支持跟随系统/浅色/深色，调整分类图标、状态颜色与过期提示对比度；模拟器已点击验证深色主题。
- [x] 支持大字体、长文本、横屏、平板和分屏，修复固定卡高及悬浮底栏遮挡；新增内容使用可滚动/可换行布局。
- [x] 完成 TalkBack 顺序与语义、触控尺寸、焦点、状态播报及非颜色提示检查；错误和过期摘要使用 live region/文字说明。
- [x] 覆盖本地/远程、空态/错误态、表单、归档与过期提示的界面回归；模拟器验证远程→本地隔离、工具入口和设置页。

- [x] 基于 OpenDesign 方案统一 SuirenX UI 视觉语言。2026-09-07：资产首页采用黑色总览卡、荧光绿主行动色、圆角资产卡与筛选胶囊；详情页补充持有/日均成本/保修状态指标；新建与编辑表单按基本信息、生命周期、备注分组；工具箱与设置页统一卡片层级；底部浮动导航和新增入口统一为同一套强调色。未改变资产、同步、归档和保质期业务数据语义。

### M5：身份、跨设备同步与远程离线能力

- [x] 设计身份认证、数据归属、授权隔离与退出登录后设备数据保留规则。见 `docs/data-sync.md`。
- [x] 设计稳定跨设备 ID、版本号、删除标记、幂等、增量同步与冲突解决；先明确规则再实现远程 offline-first。
- [x] 设计本地数据显式迁入远程的预览、去重、失败回滚及备份流程，不因切换模式自动上传。
- [x] 确定资产及用品的同步范围、远程离线提醒来源与时效标记。
- [x] 实现服务端账号注册/登录/退出、bcrypt 密码哈希、bearer token、owner 隔离和版本化资产/用品同步接口；HTTP/SQLite 集成测试覆盖幂等、冲突、tombstone、双 cursor 与账号隔离。2026-09-06
- [x] 普通远程资产 CRUD 写入同步事件，避免通过非同步 API 修改后增量游标遗漏变更；生产启动不再创建无主匿名示例数据。2026-09-06
- [x] Android 远程设置接入注册、登录、退出，并按服务器 origin 隔离 bearer token，切换服务器不会向新服务器泄漏旧凭据。2026-09-06
- [x] Android 远程资产接入独立 Room cache/outbox、双游标同步、稳定 UUID、在线立即提交和断网保留/下次访问重试；缓存按服务器及账号隔离，不回退到本地库。2026-09-06
- [x] 实现并验证跨设备同步、断网重试与服务端备份恢复演练。2026-09-06：Android 资产/用品独立 cache+outbox、双游标、账号隔离、冲突选择和显式本地迁入已完成；Go 集成测试覆盖并发编辑冲突、断线后 outbox 可重试语义、重复请求、tombstone、账号隔离，以及复制 SQLite 数据库后重新打开并拉取的恢复演练。两台真实设备 UI 复测仍需外部设备环境。

## P2：后续工程与工具箱扩展

- [ ] 搜索、排序、分类和组合筛选（本轮先支持资产标签录入与用品到期筛选）。
- [ ] 更多 ViewModel、Repository、DTO 映射和 Compose 关键状态测试（本轮新增业务随功能补测）。
- [ ] 统一错误模型、请求 ID、结构化日志和超时配置。
- [ ] Hilt 从 kapt 迁移到 KSP，启用 AGP 9 内置 Kotlin 并移除兼容开关。
- [ ] 数据导入导出扩展（如 CSV）；本地完整备份恢复已提前至 M1。
- [ ] 原生 iOS 应用（Swift + SwiftUI）。
- [ ] Web 管理界面。
- [ ] SQLite 到 PostgreSQL 的迁移演练。
- [ ] 选择边界清晰的计算模块用 C++ 重写，通过契约测试对比 Go 实现。
- [ ] CI：Go 测试、Proto 校验、Android 构建和静态检查。
- [ ] Release 签名、版本策略、隐私说明与分发流程。
- [ ] 同步 README 的接口表、创建示例和启动说明。

心愿清单、独立趋势页与月度趋势图表已从当前路线移除；已有资产总览保留。

## 已完成

- [x] 价格字体改用原稿品牌规范中的 Android 替代字体 Roboto Mono，打包 regular/medium 数字子集和 OFL 许可，统一总览、资产金额和详情的字形，并恢复指标数字的近黑颜色。2026-09-07。
- [x] 补齐资产卡片购入价格的 Roboto Mono 字体，按原稿校准价格字距，日均价格的“/天”使用小号正文单位。2026-09-07。

- [x] 校准 OpenDesign 总览卡片：恢复金额旁标注、双灰底指标块、紧凑行高和底部信息栏；资产页补齐指标分隔线，资产入口使用线框收纳盒，手机改为圆角正面矢量图标，保持原 icon_key。2026-09-07。

- [x] 按 OpenDesign `suirenx-asset-redesign.html` 落地原生 Android UI：独立总览、总览/资产/工具/设置四栏、浅色细边框资产卡与滚动紧凑总览、名称/标签搜索、详情与图标选择表单、用品近期提醒；沿用真实资产/用品数据和现有保存、归档、同步流程。Debug 构建与 27 项资产回归测试通过；模拟器验证新增、搜索、编辑图标保存、返回保留 Tab、用品入口与浅深色显示。2026-09-07。
...
- [x] 四个主页面共用燧人顶部栏：统一状态栏下 18dp 留白、48dp 最小高度与标题行高；资产页总金额对齐总览页的 28sp / Medium 数字样式。2026-09-07。
- [x] 设置页按 OpenDesign 改为数据、外观、提醒、关于分组列表；数据来源支持本地/服务端选择，地址与账号迁入管理收进二级页，保留备份恢复、主题和临期天数入口，沿用现有数据隔离逻辑。2026-09-07。
- [x] 修复选择服务端后底栏消失：首次配置仅检查是否选择数据来源，未登录或未配置地址不再替换主导航，服务端配置继续使用设置 Tab。2026-09-07。

- [x] 页面导航按 OpenDesign 原稿直接切换：禁用两层 NavHost 的默认淡入淡出，移除详情/表单滑入滑出，统一 Tab、详情、编辑和用品的前进/返回过渡。2026-09-07。

- [x] 补齐 Navigation 2.10 手势返回的独立过渡配置：两层 NavHost 禁用 predictivePopEnter/Exit 的默认淡入与缩放，使用品等页面手势返回与点击返回一致。2026-09-07。
- [x] 用品/保质期页按 OpenDesign 重建：工具详情页头、介绍标题、数字提醒总览、图标/说明/日期横向卡片，统一整页滚动并保留底部导航；新增、筛选、编辑、归档和同步处理沿用真实业务。2026-09-07。
- [x] 对齐新版用品稿：状态概览展示总记录、关注数及三列状态指标；列表分需要处理/后续提醒（归档及已处理记录保留独立展示），移除今日到期筛选并将当日项目纳入临期；底栏选项 48dp、整体 64dp，图标 20dp、文字 10sp。2026-09-07。
- [x] 工具页对齐新版 OpenDesign：紧凑横向入口、42dp 图标容器、待处理标记、更多工具占位和底部说明；用品待处理数接入真实状态，新增入口保留在用品页。2026-09-07。

- [x] 统一四个主页面「燧人」顶部位置：`SuirenHeader` 统一管理状态栏下 14 dp 间距，以设置页为基准，移除各页面重复顶部留白。2026-09-08。

- [x] 2026-09-08 移除资产与用品列表的同步状态/冲突卡片，同步错误不再显示在设置首页底部；保留设置二级页的状态处理入口。
- [x] AGENTS.md 增加设计与数据能力有偏差时先更新 OpenDesign，再改 App/UI 的规则。
- [x] 2026-09-09 使用 OpenDesign Local Codex 更新现有原型后，对齐「同步设置和状态」「账号和服务器」及地址删除确认页。删除当前连接会停止同步、清除该地址凭证且不自动选择其他服务器；删除非当前地址不影响当前连接，保留本机数据和归属绑定。Android 构建、5 项设置单元测试和 13 项数据设备测试通过。

- [x] 2026-09-08 修复用品页系统返回与底栏点击交错时重复出栈导致空白主页；构建及模拟器资产→工具→用品→设置路径验证通过。

- [x] 2026-09-09 排查真机缺少服务器删除入口：手机仍安装 9 月 8 日旧版，已构建并覆盖安装包含地址删除的新版，保留应用数据。

- [x] 2026-09-09 先更新 OpenDesign 提示位置，再移除服务器/同步页底部重复错误；错误只在对应卡片显示，删除当前连接清理首次同步失败状态。构建和 6 项设置单元测试通过，已覆盖安装到真机。

- [x] 2026-09-10 先用 OpenDesign Local Codex 更新原稿，再取消旧服务器/账号绑定阻拦：切换目标先备份业务与同步快照，重置目标版本/游标/批次，保留本机数据、tombstone 与人工冲突处理。
- [x] 2026-09-10 完善服务器与账号流程：注册/登录成功返回账号页，字段校验解释不可提交原因，密码显隐与表单清理，切换账号及连接操作互斥，迟到登录响应隔离、中文错误和离线退出。临时服务端 + 模拟器真实注册成功跳转验证通过；9 项设置单元测试、18 项数据设备测试和 Go 测试通过，最终 APK 已覆盖安装到真机（保留数据）；服务端代码未部署。

- [x] 2026-09-10 OpenDesign 原型补正后实现服务器列表→详情→所属账号；支持不切换当前目标的新增、原位编辑、重复地址拒绝、无凭证健康测试及多服务器独立登录状态迁移。
- [x] 2026-09-10 修复用品空位置被服务端拒绝造成 HTTP 400 的校验差异；增加客户端可读错误与回归测试，人工测试记录真实 HTTP 同步返回 200。
- [ ] 将上述服务端修复部署到手机当前连接的实例；等待部署方式/入口，不重放用户原始批次。

- [x] 2026-09-10 服务器详情新版 APK 已构建，21 项数据设备测试、9 项设置单元测试及 Go 测试通过；手机重连后已通过 adb install -r 覆盖安装，保留应用数据。

- [x] 2026-09-10 将工作树中服务器、账号和同步修复同步回原项目目录，保留原目录本地配置与数据；同步前保存源文件备份。
